...
def test_001_regulator_en(command):
    ...
    for regulator in regulators.keys():
        if not "dts_only" in regulators[regulator].keys():
            ...
            if bd71828.check_regulator_enable_mode(regulator, command):
                result = bd71828.regulator_enable(regulator, command)
                check_result(result)

            if bd71828.check_regulator_enable_mode(regulator, command) and not bd71828.check_regulator_always_on_mode(regulator, command):
