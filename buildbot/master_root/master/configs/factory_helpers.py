...
def doStepIf_collect_dmesg(step, product):
    projects = ["linux_rohm_devel", "linux_fast_test", "linux_local_test"]

    if step.getProperty('project') in projects or check_tag(step, product) == True:
...
def doStepIf_powerdown_beagle(step, product):
    ...
    if step.getProperty('project') in projects or step.getProperty('buildername') == 'Test_Linux' or check_tag(step, product) == True:
...
def doStepIf_generate_driver_tests(step, product, dts):
    projects = ["linux_rohm_devel", "test_linux", "linux_fast_test", "linux_local_test"]
