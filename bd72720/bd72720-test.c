...
/*
extern int bd71828_get_runlevel(struct regulator *regulator, unsigned int *level);
extern int bd71828_set_runlevel_voltage(struct regulator *regulator, unsigned int uv,
                                 unsigned int level);
*/
 ...
 \
	...
	{ \
		...
		{ \
			...
			rval = /* bd71828_set_runlevel_voltage(r, v, l) */ -ENOSYS;				    \
			if (rval)								    \
		       		pr_err("runlevel set failed %d\n", rval);			    \
		} \
	} \
	else \
		pr_err("sscanf returned %d\n", rval); \
...
\
	...
	{ \
		if(r && !IS_ERR(r)) \
			rval = /* bd71828_set_runlevel(r, *b - '0'); */ -ENOSYS;		\
		if (rval)							\
	       		pr_err("runlevel set failed %d\n", rval);		\
	} \
	if (!rval) \
...
{ \
 ...
 \
	if(r && !IS_ERR(r)) { \
 \
		rval = /* bd71828_get_runlevel(r, &rl) */ -ENOSYS; \
		if (rval)			\
			pr_err("runlevel get failed %d\n", rval); \
	} \
	if (!rval) \
		return sprintf(b,"%u\n", rl); \
 \
...
static ssize_t clk_en_store(struct kobject *ko, struct kobj_attribute *a, const char *b, size_t c)
{
	...
	if(c>1 && (*b == '1' || *b == '0'))
	{
		if(g_c && !IS_ERR(g_c))
		{
			...
			pr_err("clk_get(NULL, bd72720-32k-out) has FAILED (%d)\n",(rval=PTR_ERR(g_c)));
	};
	if(!rval && (rval=c))
		pr_info("YaY!, Clk '%s' %sbled\n","bd72720-32k-out",(*b == '0')?"disa":"ena");
	else
...
//	g_c = clk_get(NULL, "bd71828-32k-out");
	g_c = clk_get(md.this_device, "bd72720-32k-out");
	return rv;
};
